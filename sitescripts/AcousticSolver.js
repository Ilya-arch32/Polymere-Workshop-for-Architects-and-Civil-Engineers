...
export class AcousticSolver {
    ...
    rirDuration = 4.0; // seconds (343m/s * 4.0s = 1372m max propagation, enough for large rooms)
    rirData = null;
    rirRecorded = false;
    ...
    async initialize(voxelBuffer) {
        ...
        // Uniforms struct with absorption_coeffs: array<f32, 16>
        // grid_size (16b) + constants (32b) + absorption_coeffs (16 elements * 16b padding = 256b) = 304 bytes total
        ...
        // WGSL array<f32, 16> in a uniform buffer has a stride of 16 bytes per element
        const absorptionData = new Float32Array(64); // 16 elements * 4 floats per element
        for (const mat of Object.values(MATERIAL_ABSORPTION)) {
            if (mat.id < 16) {
                absorptionData[mat.id * 4] = mat.alpha; // Store at the start of each 16-byte aligned block
            }
        }
        ...
        this.device.queue.writeBuffer(this.uniformBuffer, 48, absorptionData); // offset 48
        console.log(`[AcousticSolver] Uniforms: resolution=${resolution}m, grid=${nx}x${ny}x${nz}, initialized absorption coefficients`);
    ...
    async injectImpulse(x, y, z) {
        ...
        console.log(`[AcousticSolver] Injecting spatial Gaussian impulse near (${safeX}, ${safeY}, ${safeZ})`);
        ...
        for (let dz = -radius; dz <= radius; dz++) {
            for (let dy = -radius; dy <= radius; dy++) {
                let currentY = safeY + dy;
                let currentZ = safeZ + dz;
                
                if (currentY < 0 || currentY >= ny || currentZ < 0 || currentZ >= nz) continue;
                
                let startX = Math.max(0, safeX - radius);
                let endX = Math.min(nx - 1, safeX + radius);
                let rowLength = endX - startX + 1;
                ...
                for (let dx = startX - safeX; dx <= endX - safeX; dx++) {
                    const r2 = dx*dx + dy*dy + dz*dz;
                    if (r2 <= radius * radius) {
                        rowData[dx - (startX - safeX)] = 1000.0 * Math.exp(-r2 / 2.0);
                        hasData = true;
                    }
                }
                
                if (hasData) {
                    const startIdx = startX + currentY * nx + currentZ * nx * ny;
                    if (startIdx >= 0 && startIdx + rowLength <= maxIdx) {
                        this.device.queue.writeBuffer(this.pressureBufferA, startIdx * 4, rowData);
                    }
                }
            }
        }
    ...
    async recordRIR(sourceX, sourceY, sourceZ, onProgress = null, numSteps = null) {
        ...
        // Calculate receiver position: 2.0m from source (or use default if manual set)
        const desiredOffset = Math.floor(2.0 / res); // 2 meters in voxel units
        let recX, recY, recZ;
        ...
        if (isDefaultReceiver) {
            ...
            if (srcVoxelX < nx / 2) {
                // Source is in the left half, place receiver to the right
                recX = Math.max(1, Math.min(nx - 2, srcVoxelX + desiredOffset));
            } else {
                // Source is in the right half, place receiver to the left
                recX = Math.max(1, Math.min(nx - 2, srcVoxelX - desiredOffset));
            }
            recY = srcVoxelY; // Same Y
            recZ = srcVoxelZ; // Same Z
            console.log(`[AcousticSolver] Auto-positioned receiver ${desiredOffset} voxels from source`);
        } else {
    ...
    generateFDTDShaders() {
        return `
            struct Uniforms {
                ...
                _pad2: f32,
                _pad3: f32,
                absorption_coeffs: array<f32, 16>,
            }
            ...
            fn updatePressure(@builtin(global_invocation_id) gid: vec3<u32>) {
                ...
                if (is_solid_only) {
                    ...
                    // Fetch material ID of the current solid cell
                    let mat_id = u32(voxels[idx * 8u + 1u]);
                    
                    // Default alpha to 0.02 (concrete) if out of bounds
                    var alpha = 0.02; 
                    if (mat_id < 16u) {
                        alpha = uniforms.absorption_coeffs[mat_id];
                    }
                    
                    let reflection_coeff = 1.0 - alpha; // 1.0 = perfect reflection, 0.0 = full absorption
