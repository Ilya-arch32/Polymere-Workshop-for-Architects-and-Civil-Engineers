...
export class CHTSolver {
    ...
    async initialize(voxelStateBuffer, velocityBuffer, initialTemperature) {
        ...
        // 80: moldRiskSteps
        // 84: solar_irradiance (f32)
        // 88-92: PAD (align solar_direction to 16-byte boundary at 96)
        // 96-108: solar_direction (vec3<f32>)
        // 108-112: implicit padding (struct total = 112 bytes)
        // Buffer allocated at 144 bytes (>= 112, multiple of 16) for safety
        const uniformSize = 144; // >= WGSL struct size (112 bytes)
        ...
        view.setFloat32(24, 2.2e-5, true); // alpha_solid (concrete thermal diffusivity)
        // alpha_fluid: EFFECTIVE turbulent eddy diffusivity for room air mixing
        // Molecular air diffusivity is 2.2e-5 m²/s, but LBM velocity field is frozen
        // during CHT simulation, so advective transport is near-zero. We use an
        // effective eddy diffusivity (~225x molecular) to approximate the natural
        // convection mixing that would occur in a heated room.
        view.setFloat32(28, 0.005, true);  // alpha_fluid (effective eddy diffusivity)
        ...
        // NEW: Solar interior absorption pipeline (ASHRAE 140 SHGC transmission)
        // Traces solar rays through WINDOW voxels to deposit heat on interior surfaces
        this.solarInteriorPipeline = this.device.createComputePipeline({
            label: 'Solar Interior Absorption Pipeline',
            layout: pipelineLayout,
            compute: {
                module: shaderModule,
                entryPoint: 'solar_interior_absorption'
            }
        });
        console.log('[CHTSolver] Solar interior absorption pipeline created (SHGC=0.787)');
        ...
        // ============================================
        // DIAGNOSTIC: Count VOXEL_WINDOW instances
        // ============================================
        try {
            const voxelReadSize = voxelStateBuffer.size;
            const stagingBuf = this.device.createBuffer({
                size: voxelReadSize,
                usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ
            });
            const cmdEnc = this.device.createCommandEncoder();
            cmdEnc.copyBufferToBuffer(voxelStateBuffer, 0, stagingBuf, 0, voxelReadSize);
            this.device.queue.submit([cmdEnc.finish()]);
            await stagingBuf.mapAsync(GPUMapMode.READ);
            const voxelData = new Float32Array(stagingBuf.getMappedRange());

            let windowCount = 0, solidCount = 0, fluidCount = 0, glassCount = 0;
            let windowMatCount = 0; // material_id == 3 (GLASS material)
            const stride = 8; // 8 floats per voxel
            const total = voxelData.length / stride;

            for (let i = 0; i < total; i++) {
                const state = Math.round(voxelData[i * stride]);
                const material = Math.round(voxelData[i * stride + 1]);
                if ((state & 16) !== 0) windowCount++;  // VOXEL_WINDOW = 16
                if ((state & 4) !== 0) glassCount++;     // VOXEL_GLASS = 4
                if ((state & 1) !== 0) solidCount++;     // VOXEL_SOLID = 1
                if ((state & 2) !== 0) fluidCount++;     // VOXEL_FLUID = 2
                if (material === 3) windowMatCount++;    // MaterialID.GLASS
            }

            console.log('[CHTSolver] 🔍 VOXEL STATE DIAGNOSTIC:');
            console.log(`[CHTSolver]   SOLID:  ${solidCount} voxels`);
            console.log(`[CHTSolver]   FLUID:  ${fluidCount} voxels`);
            console.log(`[CHTSolver]   GLASS:  ${glassCount} voxels (state & 4)`);
            console.log(`[CHTSolver]   WINDOW: ${windowCount} voxels (state & 16)`);
            console.log(`[CHTSolver]   Glass material (id=3): ${windowMatCount} voxels`);

            if (windowCount === 0) {
                console.warn('[CHTSolver] ⚠️ ZERO WINDOW voxels! Solar interior absorption will not work.');
                console.warn('[CHTSolver]   Check VoxelizerGPU.wgsl: glass triangles should set state=21 (SOLID|GLASS|WINDOW)');
            } else {
                console.log(`[CHTSolver] ✅ ${windowCount} WINDOW voxels detected - solar transmission enabled`);
            }

            stagingBuf.unmap();
            stagingBuf.destroy();
        } catch (e) {
            console.warn('[CHTSolver] Voxel state diagnostic failed:', e.message);
        }
    }
    ...
    async step(dt) {
        ...
        boundaryPass.end();
        // 4. Solar interior absorption (SHGC transmission through windows → floor heating)
        const solarPass = commandEncoder.beginComputePass({ label: 'Solar Interior Absorption Pass' });
        solarPass.setPipeline(this.solarInteriorPipeline);
        solarPass.setBindGroup(0, bindGroup);
        solarPass.dispatchWorkgroups(dispatchX, dispatchY, dispatchZ);
        solarPass.end();
        // 5. Compute buoyancy forces for LBM
        ...
        buoyancyPass.end();
        // 6. Vapor diffusion (ISO 13788 moisture transport)
        ...
        // 7. Mold risk assessment (ISO 13788 surface condensation)
