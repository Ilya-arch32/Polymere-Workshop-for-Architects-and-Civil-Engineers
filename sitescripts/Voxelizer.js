...
import { MaterialID, ASHRAE_MATERIALS_DB } from './VoxelTypes.js';
/**
...
export class Voxelizer {
    ...
    extractTriangles() {
        ...
        this.scene.traverse((object) => {
            if ((object.isMesh || object.type === 'Mesh') && object.geometry) {
                ...
                const material = this.resolveASHRAEMaterial(mesh, '', '');

                // ============ DIAGNOSTIC: Analyze mesh type ============
                const meshName = (mesh.name || '').toLowerCase();
                const userData = mesh.userData || {};
                let ifcType = (userData.type || userData.ifcType || '').toUpperCase();
                const expressID = userData.expressID || 'unknown';

                // ============================================================
                // IFC API ENRICHMENT: Query real IFC type & name via expressID
                // web-ifc-three often leaves mesh.name and userData.type EMPTY
                // for elements like IfcPlate. The ONLY reliable source is the
                // IFC API itself via GetLine().
                // ============================================================
                let ifcRealName = '';  // Enriched IFC element name
                let ifcRealType = '';  // Enriched IFC type (e.g. "IFCPLATE")

                if (expressID !== 'unknown' && window.ifcAPI && window.currentModelID !== undefined) {
                    try {
                        const line = window.ifcAPI.GetLine(window.currentModelID, parseInt(expressID));
                        if (line) {
                            const typeID = line.type;
                            ifcRealType = (window.ifcAPI.GetNameFromTypeCode
                                ? window.ifcAPI.GetNameFromTypeCode(typeID)
                                : '').toUpperCase();
                            ifcRealName = (line.Name?.value || '').toLowerCase();
                        }
                    } catch (_e) { /* IFC query failed, use fallback */ }
                }

                // Merge enriched IFC data with Three.js mesh data
                // This ensures keywords match even when mesh.name/userData are empty
                const enrichedName = meshName + ' ' + ifcRealName;
                const enrichedType = ifcType || ifcRealType;

                // Count by IFC type
                const typeToTrack = enrichedType || ifcType;
                if (typeToTrack) {
                    ifcTypeStats[typeToTrack] = (ifcTypeStats[typeToTrack] || 0) + 1;
                }

                // Categorize element - ROBUST with IFC API enrichment
                let category = 'OTHER';
                const nameLower = enrichedName.toLowerCase();
                const typeUpper = enrichedType.toUpperCase();

                // WINDOW detection - MUST be checked BEFORE WALL (curtain walls contain 'wall')
                // Handles: IfcWindow, IfcPlate, IfcCurtainWall, curtain wall panels, storefronts
                const isGlazingName = (
                    nameLower.includes('window') || nameLower.includes('glass') ||
                    nameLower.includes('glazing') || nameLower.includes('glazed') ||
                    nameLower.includes('storefront') || nameLower.includes('skylight') ||
                    nameLower.includes('transparent') || nameLower.includes('curtain') ||
                    // Russian glazing keywords (Revit RU exports)
                    nameLower.includes('остеклен') || nameLower.includes('витраж') ||
                    nameLower.includes('витрин') || nameLower.includes('стекл') ||
                    nameLower.includes('панель') || nameLower.includes('окн') ||
                    // EU translations
                    nameLower.includes('vitrage') || nameLower.includes('verglas') ||
                    nameLower.includes('fenster') || nameLower.includes('ventana')
                );
                const isWindowType = (
                    typeUpper.includes('WINDOW') ||
                    typeUpper.includes('CURTAINWALL') ||
                    typeUpper.includes('CURTAIN_WALL') ||
                    // IfcPlate in building IFC = curtain wall glass panel (always)
                    typeUpper.includes('PLATE')
                );

                if (isWindowType || isGlazingName) {
                    category = 'WINDOW';
                    windowCount++;
                } else if (typeUpper.includes('WALL') || nameLower.includes('wall') || nameLower.includes('wand') || nameLower.includes('mur') || nameLower.includes('pared') || nameLower.includes('стен')) {
                    category = 'WALL';
                    wallCount++;
                } else if (typeUpper.includes('DOOR') || nameLower.includes('door') || nameLower.includes('дверь')) {
                    category = 'DOOR';
                    doorCount++;
                ...
                if (indices) {
                    ...
                    for (let i = 0; i < indices.count; i += 3) {
                        ...
                        // material_id maps to ASHRAE database: 3=Glass(window), or resolved ID
                        const matId = (category === 'WINDOW' || category === 'OPENING')
                            ? MaterialID.GLASS  // Windows always get glass material (3)
                            : this.resolveASHRAEMaterial(mesh, enrichedName, typeUpper);
    ...
    detectIFCElementType(mesh) {
        ...
        // Windows: IfcWindow, IfcPlate, IfcCurtainWall, curtain wall panels, storefronts
        const isGlazingName = (
            name.includes('window') || name.includes('glass') ||
            name.includes('glazing') || name.includes('glazed') ||
            name.includes('storefront') || name.includes('skylight') ||
            name.includes('transparent') || name.includes('curtain') ||
            name.includes('окно') || name.includes('окн') ||
            name.includes('остеклен') || name.includes('витраж') ||
            name.includes('витрин') || name.includes('стекл') ||
            name.includes('панель') ||
            name.includes('vitrage') || name.includes('verglas') ||
            name.includes('fenster') || name.includes('ventana')
        );
        if (ifcType.includes('window') ||
            ifcType.includes('curtainwall') || ifcType.includes('curtain_wall') ||
            ifcType.includes('plate') ||
            isGlazingName) {
            console.log(`[Voxelizer] Detected WINDOW: ${mesh.name} (IFC: ${ifcType})`);
            return {
    ...
    /**
     * ASHRAE Material Resolution — Three-tier system
     * Tier 1: IFC native property extraction (IfcMaterialProperties)
     * Tier 2: Bilingual string matching against ASHRAE_MATERIALS_DB keywords
     * Tier 3: IFC class-based fallback (IfcWall→Plasterboard, IfcSlab→Concrete, etc.)
     * @param {Object} mesh - Three.js mesh object
     * @param {string} enrichedName - IFC-enriched name string (may include IFC Name)
     * @param {string} ifcType - IFC entity type (e.g. 'IFCWALLSTANDARDCASE')
     * @returns {number} MaterialID matching ASHRAE_MATERIALS_DB key
     */
    resolveASHRAEMaterial(mesh, enrichedName, ifcType) {
        const meshName = (mesh.name || '').toLowerCase();
        const searchString = (meshName + ' ' + enrichedName).toLowerCase();
        const typeStr = (ifcType || '').toUpperCase();

        // ────────────────────────────────────────────
        // Tier 1: IFC Property Extraction
        // Query IfcRelAssociatesMaterial → IfcMaterial → thermal props
        // ────────────────────────────────────────────
        const expressID = mesh.userData?.expressID;
        let ifcRealType = typeStr; // Use provided type, enrich via API if possible

        if (expressID && window.ifcAPI && window.currentModelID !== undefined) {
            try {
                const line = window.ifcAPI.GetLine(window.currentModelID, parseInt(expressID));
                if (line) {
                    // Enrich IFC type from API if not already available
                    if (!ifcRealType && line.type) {
                        ifcRealType = (window.ifcAPI.GetNameFromTypeCode
                            ? window.ifcAPI.GetNameFromTypeCode(line.type)
                            : '').toUpperCase();
                    }
                    // Try to read material name from IFC
                    const ifcMatName = (line.Name?.value || '').toLowerCase();
                    if (ifcMatName) {
                        const combinedSearch = searchString + ' ' + ifcMatName;
                        const tier1Result = this._matchASHRAEKeywords(combinedSearch);
                        if (tier1Result !== null) {
                            return tier1Result;
                        }
                    }
                }
            } catch (_e) { /* IFC query failed, fall through to Tier 2 */ }
        }

        // ────────────────────────────────────────────
        // Tier 2: String Matching Fallback
        // Match against ASHRAE_MATERIALS_DB bilingual keywords  
        // ────────────────────────────────────────────
        const tier2Result = this._matchASHRAEKeywords(searchString);
        if (tier2Result !== null) {
            return tier2Result;
        }

        // ────────────────────────────────────────────
        // Tier 3: IFC Class-Based Fallback
        // Guess material from IFC entity type when name has no keywords
        // Handles generic Revit names like 'Базовая стена:Типовой - 200мм'
        // ────────────────────────────────────────────
        if (ifcRealType) {
            if (ifcRealType.includes('WALL')) {
                // Walls default to Plasterboard for ASHRAE 140 low-mass compliance
                return MaterialID.PLASTERBOARD; // ID 9, ρ=950, cp=840
            }
            if (ifcRealType.includes('SLAB') || ifcRealType.includes('FLOOR')) {
                return MaterialID.CONCRETE; // ID 1, ρ=2400, cp=880
            }
            if (ifcRealType.includes('PLATE') || ifcRealType.includes('CURTAINWALL')) {
                return MaterialID.GLASS; // ID 3, ρ=2500, cp=840
            }
            if (ifcRealType.includes('DOOR')) {
                return MaterialID.WOOD; // ID 2, ρ=600, cp=1700
            }
            if (ifcRealType.includes('MEMBER') || ifcRealType.includes('BEAM') || ifcRealType.includes('COLUMN')) {
                return MaterialID.STEEL; // ID 5, ρ=7850, cp=500
            }
            if (ifcRealType.includes('ROOF')) {
                return MaterialID.PLASTERBOARD; // ID 9 (low-mass default)
            }
        }

        // Ultimate fallback: Heavy Concrete
        return MaterialID.CONCRETE;
    }

    /**
     * Match a search string against ASHRAE_MATERIALS_DB keywords
     * Returns the best-matching material_id or null if no match
     * Priority order ensures specific materials match before generic ones
     */
    _matchASHRAEKeywords(searchString) {
        // Priority order: check specific materials first, then generic
        // (e.g. 'fiberglass' should match Fiberglass, not Glass)
        const priorityOrder = [10, 11, 15, 4, 8, 9, 13, 12, 14, 5, 6, 7, 3, 2, 1, 99];

        for (const matId of priorityOrder) {
            const matDef = ASHRAE_MATERIALS_DB[matId];
            if (!matDef) continue;
            for (const keyword of matDef.keywords) {
    ...
    /**
     * Backward-compatible wrapper (deprecated — use resolveASHRAEMaterial)
     */
    inferMaterial(mesh) {
        return this.resolveASHRAEMaterial(mesh, '', '');
    }
