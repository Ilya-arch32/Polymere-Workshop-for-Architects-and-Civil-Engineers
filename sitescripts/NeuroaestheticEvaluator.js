...
export class NeuroaestheticEvaluator {
    ...
    async evaluate(voxelBuffer, onProgress = null) {
        ...
        // 2. Spatial Cognition (Space Syntax) - Made Optional
        let spatialIntelligibility = 0.5;
        let prospectRefuge = 0.5;
        let mysteryComplexity = 0.5;

        if (this.spaceSyntaxSolver) {
            try {
                const spaceMetrics = await this.spaceSyntaxSolver.analyze(voxelBuffer);
                if (spaceMetrics && spaceMetrics.status !== 'pending' && spaceMetrics.status !== 'empty' && spaceMetrics.globalIntegration > 0) {
                    spatialIntelligibility = spaceMetrics.intelligibility || 0.5;
                    prospectRefuge = this.calculateProspectRefuge(spaceMetrics);
                    mysteryComplexity = spaceMetrics.visualComplexity || 0.5;
                } else {
                    console.log('[NeuroaestheticEvaluator] SpaceSyntax is pending/empty. Using baseline spatial metrics.');
                }
            } catch (e) {
                console.warn('[NeuroaestheticEvaluator] SpaceSyntaxSolver failed. Using baseline spatial metrics. Error:', e.message);
            }
        } else {
            console.log('[NeuroaestheticEvaluator] SpaceSyntaxSolver not provided. Using baseline spatial metrics.');
        }
